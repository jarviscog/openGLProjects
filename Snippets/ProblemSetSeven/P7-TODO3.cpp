...
int main( int argc, char* argv[])
{
    ...
    std::string FragmentShaderCode = "\
        ...
        \
        vec4 color1 = texture(tex1, cords);\
        vec4 color2 = texture(tex2, cords);\
		\
        void main() {\n\
                FragColor = blend1*tex1 + blend2+tex2;\n\
		}\n";
    ...
    if ( FInfoLogLength > 0 ){
        std::vector<char> FragShaderErrorMessage(FInfoLogLength+1);
        glGetShaderInfoLog(FragmentShaderID, FInfoLogLength, NULL, &FragShaderErrorMessage[0]);
